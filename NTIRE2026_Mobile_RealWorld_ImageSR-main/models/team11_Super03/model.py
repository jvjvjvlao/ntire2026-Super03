...
class Net(nn.Module):
    ...
    #     # 注册 hook
    #     self._features = {}
    #     self.unet.down_blocks[0].register_forward_hook(self._save_down0)

    #     self.mlp =  build_mlp(240, 2048, 768)

    # def _save_down0(self, module, input, output):
    #     # output 就是 down_blocks[0] 的输出
    #     self._features["down0"] = output


    def forward(self, x):
        # x_copy = x.clone()
        x = self.body(x)
        # down0 = self._features["down0"].permute(0,2,3,1).reshape(x.shape[0],-1,240)
        # print("x_copy.shape",x_copy.shape)
        # print("down0.shape",self._features["down0"].shape)
        return x #, self.mlp(down0)